...
import { InitBaseResponse, TypeAction, TypeSubject, type AdminSentTransmissionModel, type BaseResponse, type CorrespondenceRequest, type DocumentTransmissionModel, type SentTransmissionModel } from '@/models';
...
export const useCorrespondenceStore = () => {
  ...
  const [dataAllSent, setDataAllSent] = useState<BaseResponse<AdminSentTransmissionModel>>(InitBaseResponse);
  ...
  const getAllSentCorrespondences = async (page: number = 1, limit: number = 10) => {
    try {
      requirePermission(TypeAction.read, TypeSubject.sentCorrespondenceAll);
      const res = await coffeApi.get(`/${baseUrl}/sent/all?page=${page}&limit=${limit}`);
      const { data, meta } = res.data;
      setDataAllSent({ ...meta, data });
    } catch (error) {
      throw handleError(error);
    }
  }
