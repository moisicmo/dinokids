...
import { confirmDialog } from '@/store/alertUI';
...
export const useStaffStore = () => {
    ...
    try {
      ...
      if (id === userId) {
        await confirmDialog({
          ...
          description: 'Tu rol o sucursales cambiaron. Debes volver a iniciar sesión para aplicar los cambios.',
          confirmText: 'Volver a iniciar sesión',
          hideCancel: true,
        });
        startLogout();
        return;
  ...
  const deleteStaff = async (id: string) => {
    try {
      ...
      if (id === userId) {
        showError('Acción no permitida', 'No puedes eliminar tu propio usuario.');
        return;
      }
