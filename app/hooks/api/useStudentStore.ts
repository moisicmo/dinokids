...
export const useStudentStore = () => {
  ...
  const getStudents = async (page: number = 1, limit: number = 10, keys: string = '', status: string = '') => {
    try {
      requirePermission(TypeAction.read, TypeSubject.student);
      const res = await coffeApi.get(`/${baseUrl}?page=${page}&limit=${limit}&keys=${keys}&status=${status}`);
