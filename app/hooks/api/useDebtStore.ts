...
import { InitBaseResponse, TypeAction, TypeSubject, type BaseResponse, type DebtModel, type StudentModel } from '@/models';
...
export const useDebtStore = () => {
  ...
  const [dataStudentsWithDebt, setDataStudentsWithDebt] = useState<BaseResponse<StudentModel>>(InitBaseResponse);
  ...
  const getStudentsWithDebt = async (page: number = 1, limit: number = 10, keys: string = ''): Promise<void> => {
    try {
      requirePermission(TypeAction.read, TypeSubject.debt);
      const res = await coffeApi.get(`/${baseUrl}/students?page=${page}&limit=${limit}&keys=${keys}`);
      const { data, meta } = res.data;
      const payload: BaseResponse<StudentModel> = { ...meta, data };
      setDataStudentsWithDebt(payload);
    } catch (error) {
